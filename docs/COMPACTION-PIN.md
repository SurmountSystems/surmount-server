...
**Last updated:** 2026-09-20 (full test-and-deploy:
[TEST-AND-DEPLOY.md](TEST-AND-DEPLOY.md); agent through dry-run). Prior 2026-09-11 (each public HTTP Host gets its own v3 onion;
Onion-Location is `http://{that-host-onion}{path}`; mail Hosts stay
unmapped). Prior 2026-09-08 (GitHub tracking: plan Approve opens a
...
in operator-facts Monday leftover; it is not the HTTPS cause. `/domains`
is a config union of primary names, `static_vhosts`, and extra mail
hostnames; `source` stays `config`. Do not invent leftover Namecheap
clicks. Do not MX-flip Baxter. Esplora Hosts stay off this leaf.) Prior 2026-09-02 (before asking the operator to deploy or
...
cryptoquick **after** validating lookups succeed, not that turn while
SERVFAIL. That A-lookup gate is **closed** as of 2026-09-07. `list` surfaces EmailType; `--live set-mx` fails closed while
...
apex+www; that day's production leaf was 17 certificate hostnames). Prior same
...
per-site v3 onions (2026-09-11); mail unmapped. Prior same day: mail records
...
(Onion-Location + Alt-Svc on Axum edge; live Arti unit active; per-site v3
...
| **Test and deploy procedure** | 2026-09-20: [TEST-AND-DEPLOY.md](TEST-AND-DEPLOY.md). Agent runs named eval, `just check-remote`, and `just deploy-host -- --dry-run`. Operator does not run those to find errors. Operator runs the real switch with Eternal Terminal open. Dual-pin `AGENTS.md` Deploy dry-run is agent work. |
...
| DNS legitimacy | **Earn trust on every mailbox domain** this host sends or receives through (primary `surmount.systems`, extra local domains such as `cryptoquick.com` and `baxterartworks.com`, later mailbox domains): SPF, dual DKIM TXT (Ed25519 + RSA-4096), DMARC (live mailbox policy is `p=quarantine`; do not use `p=reject`; `_dmarc.baxterartworks.com` intended is `p=quarantine`; public `_dmarc` may stay NXDOMAIN while EmailType is FWD), PTR/rDNS, MTA-STS, TLS-RPT, CAA, **and primary-class DNSSEC** (Namecheap hosted **DNSSEC Status ON**; ECDSA P-256 SHA-256 algorithm 13 acceptable for now). Working Namecheap hosted DNS when we are the registrar. No leftover parent DS without matching DNSKEY (that SERVFAILs validating resolvers; cryptoquick.com was this class). That leftover is a SERVFAIL **bug to clear then sign**, not a reason to leave the zone unsigned. We asked cryptoquick OFF only because leftover DS 2368 (alg 13, digest type 1 SHA-1) had no DNSKEY. **Live 2026-09-07:** leftover parent DS key tag 2368 is **gone**. Validating A for cryptoquick apex/www succeeds (AD true). Do **not** keep "wait for SERVFAIL to clear before adding cryptoquick web names" as the live A-lookup gate. SHA-1 parent DS digest type 1 remains standing DNSSEC quality debt in operator-facts Monday leftover; it is not the HTTPS cause. Do **not** re-add 2368 by hand. Do **not** invent leftover Namecheap clicks. Do **not** MX-flip Baxter. Intended production leaf is **20** certificate hostnames on one PEM (`with_single_cert`). Live leaf still has **18** names (CT) and still omits `cryptoquick.com` / `www.cryptoquick.com`. That mismatch is why cryptoquick HTTPS fails verify. Esplora Hosts stay off this leaf. Operator wants hosted ON same as `surmount.systems`. DS digest type 1 (SHA-1) must FAIL even if a DNSKEY exists. Path: [DNS.md](DNS.md) *Best DNSSEC we can actually run*. Do **not** harden only the primary. Static-site-only extra vhosts are **not** automatically mail domains. Operator asked the primary public MX flip 2026-08-20 (dual-sign + PTR already green). `--live set-mx` is **not** a public MX flip while Email Forwarding is on; EmailType FWD fails closed. Baxter leftover is Namecheap UI **Custom MX** only if the operator asks, then re-run live `set-mx`. `list` prints EmailType. Laptop Namecheap ClientIp = laptop egress. [DNS.md](DNS.md). |
...
| Item | Direction |
|------|-----------|
| Stack | **Axum + Leptos SSR** (invested); multi-page admin console (`/`, `/domains`, `/accounts`, `/system`, `/mail`) SSR-only; `/domains` is a config union of primary names (including derived www and mta-sts when first-class), `static_vhosts` keys, and extra mail hostnames (`source` stays `config`; not Namecheap API; not Stalwart directory); hydrate/webmail residual |
